...
export class FrameSheetPage {
  ...
  applyAttributeFilter(attributeSelection: any, numBoolSelection: any, numValSelection: any){
    ...
    for(let i = this.moveList.length -1; i >= 0; i--){
      for(let j = this.moveList[i].length -1; j >= 0; j--){ 
        ...
        if(attributeSelection.guardMin && numValSelection.guardMin < 0) {numBoolSelection.guardNegative = true;}
        if(attributeSelection.guardMax && numValSelection.guardMax > 0) {numBoolSelection.guardPositive = true;}
        if(attributeSelection.hitMin && numValSelection.hitMin < 0) {numBoolSelection.hitNegative = true;}
        if(attributeSelection.hitMax && numValSelection.hitMax > 0) {numBoolSelection.hitPositive = true;}
        if(attributeSelection.counterHitMin && numValSelection.counterHitMin < 0) {numBoolSelection.counterHitNegative = true;}
        if(attributeSelection.counterHitMax && numValSelection.counterHitMax > 0) {numBoolSelection.counterHitPositive = true;}
        ...
        // Impact Min and Max
        if(attributeSelection.impactMin && attributeSelection.impactMax){
          if (this.moveList[i][j].impact < numValSelection.impactMin || this.moveList[i][j].impact > numValSelection.impactMax) {
            this.moveList[i].splice(j, 1); 
            continue;
          }  
        }
        ...
        if(attributeSelection.guardMin && attributeSelection.guardMax){
          if (this.moveList[i][j].guard < numValSelection.guardMin || this.moveList[i][j].guard > numValSelection.guardMax) {
            this.moveList[i].splice(j, 1); 
            continue;
          }  
        }
        ...
        if(this.moveList[i][j].hit === null){
          if(!numBoolSelection.hitPositive || !numBoolSelection.hitNegative || !numBoolSelection.hitNeutral || !numBoolSelection.hitSpecial){
            this.moveList[i].splice(j, 1); 
            continue;  
          }
        }
        ...
        if(attributeSelection.hitMin && attributeSelection.hitMax){
          if (this.moveList[i][j].hit < numValSelection.hitMin || this.moveList[i][j].hit > numValSelection.hitMax) {
            this.moveList[i].splice(j, 1); 
            continue;
          }  
        }
        ...
        if(this.moveList[i][j].counterHit === null){
          if(!numBoolSelection.counterHitPositive || !numBoolSelection.counterHitNegative || !numBoolSelection.counterHitNeutral || !numBoolSelection.counterHitSpecial){
            this.moveList[i].splice(j, 1); 
            continue;  
          }
        }
        ...
        if(attributeSelection.counterHitMin && attributeSelection.counterHitMax){
          if (this.moveList[i][j].counterHit < numValSelection.counterHitMin || this.moveList[i][j].counterHit > numValSelection.counterHitMax) {
            this.moveList[i].splice(j, 1); 
            continue;
          }  
        }
